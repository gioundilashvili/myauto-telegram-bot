...
module.exports = {
  runApp: async function (dbCollection, httpClient, testItems) {
    ...
    async function getNewItems() {
      try {
        ...
        for (let page = 1; page <= 2; page++) {
          urls.push(
            `https://api2.myparts.ge/ka/search?&limit=15&man_id=5&model_id=2001&Page=${page}`
    ...
    function createMessage(item) {
      let message = `https://www.myparts.ge/ka/pr/${item.car_id}`;
      const props = {
        price_usd: "ფასი $",
        prod_year: "წელი",
        mileage: "გარბენი",
    ...
    function createImageUrls(item) {
      ...
      for (let i = 1; i <= item.pic_number; i++) {
        images.push(
          `https://static.my.ge/myparts/photos/${item.photo}/large/${item.car_id}_${i}.jpg?v=${item.photo_ver}`
        );
      }
